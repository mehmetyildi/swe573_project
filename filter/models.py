...
class UserMarketSettings(models.Model):#The market and location settings of the users are stored here
...
class Markets(models.Model):#The lookup table for market
    name=models.CharField(max_length=200,unique=True)

    def __str__(self):
        return self.name

class Locations(models.Model):#the lookup table for location. Also contains mayor's office twitter account name to look for friendship of the authors of filtered tweets.
    name=models.CharField(max_length=200,unique=True)
...
class Keywords(models.Model):#lookup for keywords.
    name=models.CharField(max_length=200,unique=True)
...
class KeywordOwner(models.Model):#keyword belongsToMany User
...
class UserAreas(models.Model):#location belongsToMany User
...
class AreaHit(models.Model):#the hit values of area related with the market location. The area value is taken through UserMarketSettings.
...
class KeywordHit(models.Model):#the hit values of keyword related with the user market. The area value is taken through UserMarketSettings.
